...
const vcard = 'BEGIN:VCARD\n'  // Gak Usah Di Ganti
            ...
            + 'FN:SlimeBot\n' //ganti nama bot lu
            + 'ORG:IRull;\n' //Ganti Jadi Nama Lu
            + 'TEL;type=CELL;type=VOICE;waid=6285607831412:+62 85607831412\n'  //ganti nomor lu! 
            + 'END:VCARD'
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6285607831412@s.whatsapp.net"]   // Ganti Nomor Lu
const pacarNumber = ["0@s.whatsapp.net"]  // Ganti Jadi Nomor Bot Lu
/************************************/

//========ATUR SEMUA D SNI===========//
namabot = 'SlimeBot' //Nama Bot lu
github = 'F' //username github
namaowner = 'Irull' //nama owner
nobot = '6285607831412' //nobot
namabot = 'SlimeBot' //namabot
igbot = 'F' //username ig
restlu = 'https://idk.com' //klo punya isi aj
gruplu = 'https://chat.whatsapp.com/L6b4QjsMpgn8vpecqZltBe' //link gc lu
        ...
        if (levelRole <= 2) {
           ...
           var prema = 'Free'
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("Mohon jaga ucapan ya kak")
                        .then(() => client.groupRemove(from, sender))
                        .then(() => {
                            client.sendMessage(from, `*「 ⚠️ ANTI BADWORD 」*\nSlimeBot akan kick kamu karena berkata kasar!`, text ,{quoted: mek})
		...
		setTimeout( () => {
			...
			switch(command) {
                ...
                case 'delbadword':  // Fix Bug By PojanGanz
                    ...
                    bad.splice(dbw)
                    fs.writeFileSync('./database/kelompok/bad.json', JSON.stringify(bad))
                    reply('⚠️Success Menghapus BAD WORD!')
                    break 
                case 'listbadword':  // Fix Bug By PojanGanz
                    let lbw = `⚠️Ini adalah list BAD WORD\nTotal : ${bad.length}\n`
                    for (let i of bad) {
                        lbw += `➸ ${i.replace(bad)}\n`
                    }
                    await reply(lbw)
				...
				case 'vnlist':  // Fix Bug By PojanGanz
					...
					case 'wa.me':
case 'wame':
if (isBanned) return reply('```Lu kebanned bang```')
...
quoted: mek
  ...
  break	
				case 'addimage':  // Fix Bug By PojanGanz
					...
					svst = body.slice(10)
					if (!svst) return reply('Nama imagenya apa kakm')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await client.downloadMediaMessage(boij)
					imagenye.push(`${svst}`)
		...
		case 'lb':
				...
				for (let i = 0; i < 10; i++) {
					...
					leaderboarduang += `*[${nom}]* ${uang[i].id.replace('@s.whatsapp.net', '')}\n◪  *Uang/Balance*: _Rp${uang[i].uang}_\n◪  *Limit*: ${limitawal - _limit[i].limit}\n`
				}
...
if (isBanned) return reply('```Lu kebanned bang```')
		...
		case 'darkneon':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} SlimeBot`)
                    txt = args.join(" ")
                    reply('[â•] Loading')
...
caption: teks, contextInfo: {
  ...
  break
		...
		case 'lpr':
					...
					client.sendMessage('6285607831412@s.whatsapp.net', options, text, {quoted: mek})
					reply('Masalah telah di laporkan ke owner SlimeBot, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		...
		case 'request':
					...
					client.sendMessage('6285607831412@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6285607831412@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER SlimeBot, Requests palsu atau main² tidak akan ditanggapi.')
					break
    ...
    case 'linkgc':
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST SEL ME BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		case 'bcc': 
				if (!isAdmin) return reply('*Only Admin bot*')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST SLIME-BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
